...
async def websocket_backend(websocket: WebSocket, url: str):
    try:
        ...
        if not video_analysis['analysis']['is_worthy']:
            ...
            await websocket.send_text(json.dumps({"step": "completed", "message": "Not worthy response generated" , "response": not_worthy_response_data}))
            await websocket.close()
            return
        else:
            ...
            await websocket.send_text(json.dumps({"step": "completed", "message": "If worthy response generated" , "response": if_worthy_response_data}))
            await websocket.close()
            return
